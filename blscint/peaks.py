...
def truncate_frame(frame, shift_freq=False, as_data=None, half_width=3, trunc_type='gaussian', peak_guess=None):
    """
    ...
    if shift_freq:
        ...
        for i in range(data.shape[0]):
            peaks[i], (offs1[i], offs2[i]) = trunc_func(data[i])
...
#         offset = int((np.max(offs2) - np.min(offs1))/2)
        ...
        for i in range(data.shape[0]):
            tr_data[i] = frame.data[i][peaks[i]-offset:peaks[i]+offset]
        peak = peaks[0]
    else:
        spec = np.mean(data, axis=0)
        peak, (_, offset) = trunc_func(spec)
        tr_data = frame.data[:, peak-offset:peak+offset]
        
    # Match frequency to truncated frame
    if frame.ascending:
        fch1 = frame.fs[peak-offset]
    else:
        fch1 = frame.fs[peak+offset]
